;(function(_myp){
    $(function(){
        ...
        var first_pop_str="<div class='pop_edit ask_first comm_ask_first' style='display: none;'><h3>提问</h3><a href='#' nodetype='close' class='close'>×</a>"+
            ...
            " class='afd_p'>更容易得到好的答案<span>&nbsp;&nbsp;</span><a href='http://ask.csdn.net/help#rule12' target='_blank'>怎样提出一个好的问题？</a></span><br>"+
            ...
            " href='http://ask.csdn.net/help' target='_blank'>问答频道提问规范</a></span></div>"+
        ...
        var second_pop_str="<form action='/upload_picture.json' id='picture_dialog_form' method='post'><input type='file' name='markdown_file' id='markdown_file' onchange='inputChange()' accept='image/gif, image/jpeg, image/png'></form><div class='pop_edit ask_second comm_ask_second'><h3>提问</h3><a href='#' nodeType='close' class='close'>×</a><div class='context'><div class='err_div'><span class='err_ico'></span>"+
            ...
            "<div class='pop_cm cm_add_img'><div class='nav-tabs'><a class='img_tab active' href='#tab_upload'>本地上传</a><a class='img_tab' href='#tab_weburl'>网络图片</a></div><div class='tab_panel active' id='tab_upload'><div class='up_img_loading'>上传中...</div><div class='set_img'><div class='markdown_file_name'></div><label for='markdown_file'>浏览</label></div><div class='markdown_file_msg'></div><div class='text-right'><span class='btn btn-default btn-sm' onclick='ajaxFileUploadToQiniu(\"editor_question\");'>上传图片</span> </div></div><div class='tab_panel' id='tab_weburl'><input type='text' placeholder='图片地址' id='af_cm_img_url'><input type='text' placeholder='图片说明' id='af_cm_img_alt'><div class='text-right'><span class='btn btn-default btn-sm' id='add_img_btn'>插入图片</span> </div></div> </div></div> "+
            "<textarea id='editor_question' rows='8'></textarea>"+
            ...
            "<div class='success'><div class='left_area'><input id='chk_cb' type='checkbox' checked='checked' disabled='disabled'/><span class='wyxs'>我要悬赏</span><select style='width: 50px' id='cb_num' class='cb_num' ><option value='5' selected='selected'>5</option><option value='10' >10</option><option value='20' >20</option><option value='40' >40</option><option value='80' >80</option><option value='160' >160</option><option value='300' >300</option><option value='500' >500</option><option value='1000' >1000</option></select>"+
            "<span class='phib_rii' style='width: 280px'><span> 币</span>   (注意：提问被删悬赏不退)</span></div><input type='hidden' id='subject_id' name='subject_id' value='0'/><a href='#' nodeType='cancel' class='cancel'>取消</a><a href='#' nodeType='ok' class='ok'>发布</a></div></div>";
        ...
        var total_ask_str=second_pop_str+common_ask_div_sc;
        ...
        $( 'body').bind( 'click', function ( evt ) {
            ...
            }else if( $target.attr( 'id') == 'up_img_btn' ){
        ...
        var coin_flag=false;
var site_domain_url = "http://ask.csdn.net";
//判断是否首次点击提问按钮
var check_accept_rules_url = site_domain_url+"/users/check_accept_rules.js";
//提问时，标题录入框的自动完成
var ask_topic_autocomplete_url = site_domain_url +"/questions/autocomplete.js";
//提问时，光标离开标题后的推荐标签
var ask_topic_tag_suggest_url = site_domain_url +"/questions/tag_autocomplete.js";
//手动输入标签时的自动完成。
var ask_label_suggest_url = "https://recommend-api.csdn.net/tag/complete?";
//提问时，发布问题的地址
var create_question_url = site_domain_url + "/questions/create_question.json";
//验证C币
var valid_coin = site_domain_url+"/users/get_scores.js";
//问答详情URL
var ask_detail_url = site_domain_url + "/questions/";
var isSubmitting = false;


        //var editor = new Editor();
        //editor.render(document.getElementById('editor_question'));





        //setTimeout($(".ask_second .CodeMirror").removeClass("CodeMirror-focused"), 3000);
        //初始化Markdown控件
        (function($) {
            var f = $;
            var i,
...
//                if (a) { prettyPrint() }
        ...
        //是否首次点击提问
        $("#float_ask_img1").bind('click', function(){
            ...
            $.ajax({
                ...
                success: function(obj) {
                    $(".ask_second,#picture_dialog_form").remove();
                    $("body").append(second_pop_str);
                    editor = new Editor();
                    $( document ).data( 'editor_question', editor );
                    editor.render(document.getElementById('editor_question'));
        ...
        $("#float_ask_img").bind('click', function(){
            $('.editor-preview.editor-preview-active').removeClass('editor-preview-active')
            $('.icon-preview.active').removeClass('active');
            //  document.domain="csdn.net";
            //pop($("body > .ask_second"));
        ...
        $(".float_subject_img").bind('click', function(){
            $('.editor-preview.editor-preview-active').removeClass('editor-preview-active')
            $('.icon-preview.active').removeClass('active');
            //  document.domain="csdn.net";
            //pop($("body > .ask_second"));
            ...
            $.ajax({
                ...
                success: function(obj) {
                    $(".ask_second,#picture_dialog_form").remove();
                    $("body").append(second_pop_str);
                    $("#ask2_tagRecomm_div, .left_area, .div_tags").hide();
        ...
        function pop($popup, isEdit){
            ...
            setTimeout( function () {
                ...
                $(".ask_second .CodeMirror").removeClass("CodeMirror-focused");
            }, 200 );
            //初始化基本的事件绑定等
            //取消事件绑定
...
//留意该接口请求类型,异步提交数据，在该次加入了验证的能力，
                    $.ajax({
                        ...
                        data: {
                            ...
                            "question[coin]": _coin,
                            "question[subject_id]": _subject_id
                        },
                        ...
                        success: function(obj) {
                            if (obj.status) {
                                ...
                                if(subject_id){
	                                ...
	                                var href = site_domain_url+"/questions/"+obj.id;
...
//提问的自动TAG,需要你自行粘贴那个JS文件
                Creat_Dynamic_Tag(".comm_ask_second #divSearchTags", ask_label_suggest_url, ".comm_ask_second input[name='txtSearchTags']");
...
function inputChange() {
      var file_name = $('#markdown_file').val();
      $('.markdown_file_name').text(file_name);
}
 function ajaxFileUploadToQiniu(editorId){
                  $('.up_img_loading').addClass('active');
                  var editor = $( document ).data(editorId);
                  ...
                  $(form).ajaxSubmit({
                      success: function (result) {
                          ...
                          if( result.status==500){
                              ...
                              //console.log(editor);
                              cmDrawImageUp(editor, result.url);
